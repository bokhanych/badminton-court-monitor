...
def send_telegram(config: Config, text: str) -> None:
    try:
        response = request_json(
            ...
            {"chat_id": config.telegram_chat_id, "text": text, "parse_mode": "HTML", "disable_web_page_preview": "true"},
        )
...
def main() -> int:
    ...
    LOG.info("Monitoring service %d, slots from %s", SERVICE_ID, config.evening_start.strftime("%H:%M"))
    while not stopping:
        ...
        deadline = time.monotonic() + max(0.0, config.interval - (time.monotonic() - started))
